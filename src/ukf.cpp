...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
  ...
  */
    ...
    if (!is_initialized_){
  	cout << "Initializing UKF: " << endl;
		    ...
		    */
				...
				x_ << x, y, 0.0, 0.0, 0.0; //use zero for v, psi, and psi_dot
		  }
    ...
    weights_(0) = lambda_ / (lambda_ + n_aug_);
  	for (int i = 1; i < 2*n_aug_ + 1; i++) { // fill weights
    	weights_(i)= 0.5 / (lambda_ + n_aug_);
  	}
  
    time_us_ = meas_package.timestamp_; //update timestamp
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  MatrixXd A = P_aug.llt().matrixL(); // square root matrix
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
  ...
  //calculate NIS
  float NIS_l = z_diff.transpose() * S.inverse() * z_diff;
  cout << "NIS_l: " << NIS_l << endl;
}
  ...
  */
  ...
  //create matrix for sigma points in measurement space
	...
	for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //2n+1 simga points
		...
		// measurement model
		Zsig(0,i) = sqrt(px_p * px_p + py_p * py_p);       //r
		Zsig(1,i) = atan2(py_p,px_p);                      //phi
		Zsig(2,i) = (px_p * v1 + py_p * v2) / Zsig(0,i);   //r_dot
	}
